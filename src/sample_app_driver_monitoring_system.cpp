...
static std::atomic<uint8_t> push_data_start(0);
...
char sound_file[40] = "";
std::string state;
std::atomic <bool> play_flag(false);


/*Structure for state trackers*/
typedef struct
{
    clock_t start_time;
    float drowsy_time;
...
{
    float speed;
    std::string head_pose[NUM_MAX_FACE];
    DrowTracker drow_tracker;
    RotateTracker rotate_tracker;
    YawnTracker yawn_tracker;
    FaceTracker face_tracker;
} FrameHandler;

void init_frame_handler(FrameHandler *frame_handler)
{
    frame_handler->drow_tracker.start_time = clock();
    frame_handler->drow_tracker.drowsy_time = 0.0;
    frame_handler->drow_tracker.color = BLUE_DATA;
    strcpy(frame_handler->drow_tracker.alarm, ATTENTION_WAV);

    frame_handler->rotate_tracker.start_time = clock();
    frame_handler->rotate_tracker.rotate_time = 0.0;
    frame_handler->rotate_tracker.color = BLUE_DATA;
    strcpy(frame_handler->rotate_tracker.alarm, DISTRACT_WAV);

    frame_handler->yawn_tracker.start_time = clock();
    frame_handler->yawn_tracker.yawning_time = 0.0;
    frame_handler->yawn_tracker.color = BLUE_DATA;
    strcpy(frame_handler->yawn_tracker.alarm, YAWN_WAV);

    frame_handler->face_tracker.start_time = clock();
    frame_handler->face_tracker.no_face_time = 0.0;
    frame_handler->face_tracker.color = RED_DATA;
    strcpy(frame_handler->face_tracker.alarm, NOT_DETECT_WAV);
}
 ...
 * Return value  : -
 ******************************************/
static void draw_skeleton(FrameHandler *frame_handler)
{
    int32_t sk_id;
    uint8_t v;
    uint8_t i;
 ...
 *               not 0 otherwise
 ******************************************/
static int8_t print_result(Image *img, FrameHandler *frame_handler)
{
    int32_t i = 0;
    ...
    if (face_count > 0)
    {
        ...
        {
            end_t = clock();
            frame_handler->yawn_tracker.yawning_time += (double)(end_t - frame_handler->yawn_tracker.start_time) / CLOCKS_PER_SEC;
            frame_handler->yawn_tracker.start_time = end_t;
            stream.str("");
            if (frame_handler->yawn_tracker.yawning_time >= LIPS_YAWN_TIMEOUT)
            {
                stream << "Yawn Detected";
                std::cout << "INFO: YAWNING - Stream state: " << stream.str() << std::endl;
                strcpy(frame_handler->yawn_tracker.alarm, YAWN_WAV);
                strcpy(sound_file, frame_handler->yawn_tracker.alarm);
            }
            str3 = stream.str();
            img->write_string_rgb(str3, TEXT_WIDTH_OFFSET, LINE_HEIGHT * 4, CHAR_SCALE_SMALL, BLUE_DATA);
        }
        if (blink_flag == 2)
        {
            end_t = clock();
            frame_handler->drow_tracker.drowsy_time += (double)(end_t - frame_handler->drow_tracker.start_time) / CLOCKS_PER_SEC;
            frame_handler->drow_tracker.start_time = end_t;
            stream.str("");

            std::cout << "Drowsiness timer: " << frame_handler->drow_tracker.drowsy_time << std::endl;

            if (frame_handler->drow_tracker.drowsy_time >= EYE_SLEEP_TIMEOUT)
            {
                stream << "Sleeping";
                std::cout << "ALERT: SLEEPING - Stream state: " << stream.str() << std::endl;
                state = "Sleeping";
                strcpy(frame_handler->drow_tracker.alarm, ALERT_WAV);
                strcpy(sound_file, frame_handler->drow_tracker.alarm);
                push_data_start.store(1);
            }
            else if (frame_handler->drow_tracker.drowsy_time >= EYE_DROW_TIMEOUT)
            {
                stream << "Drowsy Detected";
                std::cout << "DROWSINESS - Stream state: " << stream.str() << std::endl;
                strcpy(frame_handler->drow_tracker.alarm, ATTENTION_WAV);
                strcpy(sound_file, frame_handler->drow_tracker.alarm);
            }
            else
            {
                ...
                std::cout << "INFO: BLINKING - Stream state: " << stream.str() << std::endl;
            }
            str4 = stream.str();
            img->write_string_rgb(str4, TEXT_WIDTH_OFFSET, LINE_HEIGHT * 5, CHAR_SCALE_LARGE, RED_DATA);
        }
        for (i = 0; i < face_count; i++)
        {
            stream.str("");
            ...
            if (std::strcmp(random_forest_preds[i].c_str(), "CENTER") != 0) {
                end_t = clock();
                frame_handler->rotate_tracker.rotate_time += (double)(end_t - frame_handler->rotate_tracker.start_time) / CLOCKS_PER_SEC;
                frame_handler->rotate_tracker.start_time = end_t;
                stream.str("");
                std::cout << "Rotate timer: " << frame_handler->rotate_tracker.rotate_time << std::endl;
                if (frame_handler->rotate_tracker.rotate_time >= HEAD_ROTATE_TIMEOUT)
                {
                    stream << "DISTRACTION";
                    std::cout << "WARNING: DISTRACTION - Stream state: " << stream.str() << std::endl;
                    state = "DISTRACTION";
                    strcpy(frame_handler->rotate_tracker.alarm, DISTRACT_WAV);
                    strcpy(sound_file, frame_handler->rotate_tracker.alarm);
                    push_data_start.store(1);
                }
                str4 = stream.str();
                img->write_string_rgb(str4, TEXT_WIDTH_OFFSET, LINE_HEIGHT * 5, CHAR_SCALE_LARGE, YELLOW_DATA);
            } else {
                frame_handler->rotate_tracker = {
                    .start_time = clock(),
                    .rotate_time = 0.0,
                    .color = WHITE_DATA,
                };
                if (2 != blink_flag && 2 != yawn_flag) {
                    state = "ACTIVE";
                    strcpy(sound_file, "");
                    push_data_start.store(0);
                }
            }
        }
    } else {
        end_t = clock();
        frame_handler->face_tracker.no_face_time += (double)(end_t - frame_handler->face_tracker.start_time) / CLOCKS_PER_SEC;
        frame_handler->face_tracker.start_time = end_t;
        stream.str("");
        std::cout << "No_face timer: " << frame_handler->face_tracker.no_face_time << std::endl;
        if (frame_handler->face_tracker.no_face_time >= NO_FACE_TIMEOUT)
        {
            stream << "NO_FACE";
            std::cout << "ALERT: NO DRIVER - Stream state: " << stream.str() << std::endl;
            state = "NO_FACE";
            strcpy(frame_handler->face_tracker.alarm, NOT_DETECT_WAV);
            strcpy(sound_file, frame_handler->face_tracker.alarm);
            push_data_start.store(1);
        }
        str4 = stream.str();
        img->write_string_rgb(str4, TEXT_WIDTH_OFFSET, LINE_HEIGHT * 5, CHAR_SCALE_LARGE, RED_DATA);
...
/*****************************************
 * Function Name : R_Sound_Thread
 * Description   : Executes the Sound playing thread
 * Arguments     : threadid = thread identification
 * Return value  : -
 ******************************************/
void *R_Sound_Thread(void *threadid)
{
    /*Semaphore Variable*/
    int32_t sp_sem_check = 0;
    /*Variable to store the getchar() value*/
    int32_t c = 0;
    /*Variable for checking return value*/
    int8_t ret = 0;

    char run_cmd[200] = "aplay ";
    int8_t cmd_ret = 0;
    strcat(run_cmd, sound_file);
    ...
    /*Checks if sem_getvalue is executed wihtout issue*/
    errno = 0;
    ret = sem_getvalue(&terminate_req_sem, &sp_sem_check);
    if (0 != ret)
    {
        fprintf(stderr, "[ERROR] Failed to get Semaphore Value: errno=%d\n", errno);
        goto err;
    ...
    cmd_ret = system(run_cmd);
    if (0 != cmd_ret)
    {
        fprintf(stderr, "[ERROR] Failed to play alarm sound: errno=%d\n", errno);
    } else {
        sleep(0.1);
        play_flag = false;
    }
...
err:
    printf("[Sound_thread] Error occurred\n");
    sem_trywait(&terminate_req_sem);
    goto sound_p_end;

sound_p_end:
    printf("Sound Playing Thread Terminated\n");
    pthread_exit(NULL);
}
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // const uint8_t* save_buffer = img_buffer;
                if (!inference_start.load())
                {
                ...
                int sound_f_len = strlen(sound_file);
                if (sound_f_len != 0 && false == play_flag) 
                {
                    play_flag = true;
                    create_thread_sp = pthread_create(&sound_thread, NULL, R_Sound_Thread, NULL);
                    if (0 != create_thread_sp)
                    {
                        sem_trywait(&terminate_req_sem);
                        fprintf(stderr, "[ERROR] Failed to create Sound Playing Thread.\n");
                        goto err;
                    }
                }
                if (push_data_start.load()) {
                    char cwd[200];
                    ...
                    std::strftime(timestamp, sizeof(timestamp), "%d-%m-%Y_%H-%M-%S", ltm);

                    char img_name[200 + sizeof(timestamp) + sizeof(state)];
                    snprintf(img_name, sizeof(img_name), "%s/%s_%s%s", cwd, timestamp, state.c_str(), ".bmp");
                    ret = capture->save_img(img_name);
                    if (0 != ret) 
                    {
                        fprintf(stderr, "[ERROR] Failed to save image: errno=%d\n", errno);
                        goto err;
                    }
                }
            }
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    init_frame_handler(&frameHandler);
