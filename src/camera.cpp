...
int8_t Camera::save_bin(std::string filename)
{
    ...
    std::cout << "\n[debug] image buffer size: " << sizeof(uint8_t) << std::endl;
    std::cout << "\n[debug] buffer length: " << imageLength << std::endl;
    std::cout << "\n[debug] written file: " << fp << std::endl;
    ret = fwrite(buffer[buf_capture.index], sizeof(uint8_t), imageLength, fp);
    if (!ret)
    {
        printf("Writing data to binary file Failed: %d\n", ret);
        fclose(fp);
        return -1;
    }
...
* Description   : Get the capture image from buffer and save it into image file
* Arguments     : filename = image file name to be saved
* Return value  : 0 if succeeded
*                 not 0 otherwise
******************************************/
// int8_t Camera::save_img(std::string filename, const uint8_t* buffer, int32_t imageLength)
// {
//     char cwd[MAX_PATH];
...
//     char extension[5];
//     strncpy(extension, ".jpg", sizeof(extension));
//     char timestamp[20];
...
//     char img_name[MAX_PATH + sizeof(timestamp) + sizeof(filename) + sizeof(extension)];
//     snprintf(img_name, sizeof(img_name), "%s/%s_%s%s", cwd, timestamp, filename.c_str(), extension);
...
//     /*DEBUGGING*/
//     const size_t nm = sizeof(buffer) / sizeof(uint8_t);
//     ret = fwrite(buffer, sizeof(uint8_t), nm, fp);
//     // ret = fwrite(buffer, 1, imageLength, fp);
//     if (!ret)
//     {
//         fclose(fp);
//         return -1;
...
//     return 0;
// }
int8_t Camera::save_img(std::string filename)
{
    // Mat frame; 
    // cap >> frame;
    // if (frame.empty())
    ...
    // }
    // Mat resized_frame;
    // resize(frame, resized_frame, Size(CAM_IMAGE_WIDTH, CAM_IMAGE_HEIGHT), INTER_LINEAR);
    // imwrite(filename, resized_frame);
    // return 0;

    Mat yuyv_image(CAM_IMAGE_HEIGHT, CAM_IMAGE_WIDTH, CV_8UC2, buffer[buf_capture.index]);
    Mat bgra_image;
    Mat out_image(CAM_IMAGE_HEIGHT, CAM_IMAGE_WIDTH, CV_8UC4, buffer[buf_capture.index]);
    cvtColor(yuyv_image, bgra_image, COLOR_YUV2BGRA_YUYV);
    memcpy(out_image.data, bgra_image.data, CAM_IMAGE_WIDTH * CAM_IMAGE_HEIGHT * CAM_IMAGE_CHANNEL_YUY2);
    imwrite(filename, bgra_image);
    return 0;
}
